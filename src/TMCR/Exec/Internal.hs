...
module TMCR.Exec.Internal where
...
import Control.Concurrent.Async (replicateConcurrently_, Async, replicateConcurrently, withAsync)
...
runWithAsyncsForever :: (Eq (v Truthy), Eq (v County), Lattice (v Truthy), Lattice (v County), LogicValues (v Truthy) (v County)) => Int -> (forall m. MonadEval (v Truthy) (v County) m => Eval m v) -> TransactionalShuffleProgress (v Truthy) (v County) -> (TVar Int -> Async a -> IO ()) -> IO ()
runWithAsyncsForever threadCount eval progress cb = do
  v <- newTVarIO 0
  withAsync (runAsyncsForever v threadCount eval progress) $ cb v
runAsyncsForever :: (Eq (v Truthy), Eq (v County), Lattice (v Truthy), Lattice (v County), LogicValues (v Truthy) (v County)) => TVar Int -> Int -> (forall m. MonadEval (v Truthy) (v County) m => Eval m v) -> TransactionalShuffleProgress (v Truthy) (v County) -> IO a
runAsyncsForever v threadCount eval progress = do
  xs <- replicateConcurrently threadCount $ runAsyncForever v eval progress
  case xs of
    [] -> error "Must use more than 0 threads"
    (x:_) -> return x

runAsyncForever ::  (Eq (v Truthy), Eq (v County), Lattice (v Truthy), Lattice (v County), LogicValues (v Truthy) (v County)) => TVar Int -> (forall m. MonadEval (v Truthy) (v County) m => Eval m v) -> TransactionalShuffleProgress (v Truthy) (v County) -> IO a
runAsyncForever counter = runAsync' 
  (\progress -> Right <$> takeWorkItem progress counter)
...
stepsAsyncs threadCount stepCount eval progress = do
  ...
  replicateConcurrently_ threadCount $ stepsAsync v' v eval progress
  fmap (\remaining -> stepCount - remaining) $ readTVarIO v'
...
runAsync counter = runAsync' 
  (\progress -> (Right <$> takeWorkItem progress counter) <|> (Left () <$ stopWhenAllDone progress counter))
  (modifyTVar' counter $ (+ (-1)))

runShufflesAsync counter = runAsync''
  (\progress -> (Right <$> takeWorkItem' (progress ^. tShuffleQueue) counter) <|> (Left () <$ stopWhenAllDone' (progress ^. tShuffleQueue) counter))
  (modifyTVar' counter $ (+ (-1)))
  stepShuffleAsync
...
stepsAsync stepLimit counter = runAsync'
  (\progress -> (Right <$> (countStep stepLimit >> takeWorkItem progress counter)) <|> (Left () <$ (stopWhenAllDone progress counter <|> noStepsLeft stepLimit)))
  (modifyTVar' counter $ (+ (-1)))

stepsShufflesAsync stepLimit counter = runAsync''
  (\progress -> (Right <$> (countStep stepLimit >> takeWorkItem' (progress ^. tShuffleQueue) counter)) <|> (Left () <$ (stopWhenAllDone' (progress ^. tShuffleQueue) counter <|> noStepsLeft stepLimit)))
  (modifyTVar' counter $ (+ (-1)))
  stepShuffleAsync

runAsync' :: (Eq (v Truthy), Eq (v County), Lattice (v Truthy), Lattice (v County), LogicValues (v Truthy) (v County)) => (TransactionalShuffleProgress (v Truthy) (v County) -> STM (Either r ShuffleDependent)) -> STM () -> (forall m. MonadEval (v Truthy) (v County) m => Eval m v) -> TransactionalShuffleProgress (v Truthy) (v County) -> IO r
runAsync' getWork afterWork eval = runAsync'' getWork afterWork (step eval)

runAsync'' :: (TransactionalShuffleProgress (v Truthy) (v County) -> STM (Either b a)) -> STM () -> (TransactionalShuffleProgress (v Truthy) (v County) -> a -> IO ()) -> TransactionalShuffleProgress (v Truthy) (v County) -> IO b
